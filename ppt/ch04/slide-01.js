...
module.exports = {
  slideConfig: { type: "cover", index: 1, title: "技能、专家与专家团" },
  createSlide(pres) {
    ...
    s.addText("工具层 · 方法层 · 协作层——三大能力组件一次讲透", {
      x: 1.2, y: 2.98, w: 8.2, h: 0.6,
      fontSize: 15, fontFace: "Microsoft YaHei", italic: true, color: "DDF2EC", margin: 0
    });
    // 底部章节进度点（本次为第 4 章，前 4 个点亮）
    for (let i = 0; i < 9; i++) {
      s.addShape("ellipse", {
        x: 1.2 + i * 0.3, y: 4.5, w: 0.15, h: 0.15,
        fill: { color: i < 4 ? "28B894" : "FFFFFF", transparency: i < 4 ? 20 : 75 }
      });
    }
    s.addText("WorkBuddy 效率进阶实训课程 · 配套实验（实验 4.1-4.6）", {
