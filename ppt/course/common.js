...
// 分隔页（深青绿底）：大节号 + 节标题 + 要点列表
function divider(s, num, title, points) {
  s.background = { color: C.darkBg };
  topAccentBar(s);
  bottomAccentBar(s);
  s.addShape(SHAPE.RECT, { x: 0.8, y: 1.0, w: 0.05, h: 2.2, fill: { color: "B8E8DC" } });
  s.addText(num, {
...
// 深色结论条（强调页用）—— 金句字号 12，符合课程规范
function bigCallout(s, text, y, h) {
  const by = y || 2.2, bh = h || 0.38;
  ...
  s.addText(text, {
    ...
    color: C.primary, valign: "middle", margin: 0, lineSpacingMultiple: 1.15, fit: "shrink"
  });
}
