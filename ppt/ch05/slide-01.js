...
module.exports = {
  slideConfig: { type: "cover", index: 1, title: "连接器与资料库" },
  createSlide(pres) {
    ...
    s.addText("连接器架起外部系统桥梁 · 资料库让文档知识流动起来", {
    ...
    for (let i = 0; i < 9; i++) {
      s.addShape("ellipse", {
        ...
        fill: { color: i < 5 ? "28B894" : "FFFFFF", transparency: i < 5 ? 20 : 75 }
      });
    }
    s.addText("WorkBuddy 效率进阶实训课程 · 配套实验（实验 5.1-5.4）", {
