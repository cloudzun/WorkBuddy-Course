...
module.exports = {
  slideConfig: { type: "cover", index: 1, title: "模型配置" },
  createSlide(pres) {
    ...
    s.addText("为不同任务配置最合适的“大脑” · 推理模式 · 内置模型 · 自定义接入 · 积分成本", {
    ...
    for (let i = 0; i < 9; i++) {
      s.addShape("ellipse", {
        ...
        fill: { color: i < 6 ? "28B894" : "FFFFFF", transparency: i < 6 ? 20 : 75 }
      });
    }
    s.addText("WorkBuddy 效率进阶实训课程 · 配套实验（实验 6.1-6.5）", {
