...
module.exports = {
  slideConfig: { type: "cover", index: 1, title: "第7章 助理连接配置" },
  createSlide(pres) {
    ...
    s.addText("移动端发令 · 电脑端执行 · 随时随地调度 WorkBuddy", {
    ...
    for (let i = 0; i < 9; i++) {
      s.addShape("ellipse", {
        ...
        fill: { color: i < 7 ? "28B894" : "FFFFFF", transparency: i < 7 ? 20 : 75 }
      });
    }
    s.addText("WorkBuddy 效率进阶实训课程 · 配套实验（实验 7.1-7.6）", {
